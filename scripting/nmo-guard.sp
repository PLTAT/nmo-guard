...
{
	name = "NMO Guard",
	author = "PLTAT", //Dysphie
	description = PLUGIN_DESCRIPTION,
	version = PLUGIN_VERSION,
	url = "https://github.com/PLTAT/nmo-guard" //https://github.com/dysphie/nmo-guard
};
...
// 假设 MAX_BOUNDARY_ITEMS 是一个定义好的常量，表示行数
// 假设 MAX_TARGETNAME_LEN 也是一个定义好的常量，表示每行字符串的最大长度
...
void GetRecoverableItems(ArrayList arr)
{
    ...
    while ((boundary = FindEntityByClassname(boundary, "nmrih_objective_boundary")) != -1)
    {
        if (GetEntProp(boundary, Prop_Send, "m_bActive"))
        {
            ...
            // 确保 GetBoundaryItemData 返回实际填充的项数，或者知道最大项数
            // 如果 GetBoundaryItemData 总是填充 MAX_BOUNDARY_ITEMS 项，
            // 并且用空字符串表示无效项，那么下面的循环就适用。
            // 最好是 GetBoundaryItemData 返回一个 count。
            GetBoundaryItemData(boundary, names, colors);

            EntityTemplate data;
            
            // 修正循环终止条件：迭代实际的行数
            for (int i = 0; i < MAX_BOUNDARY_ITEMS; i++) // 或者使用 GetBoundaryItemData 返回的实际项数
            {
                // 修正空字符串检查：检查第一个字符是否为空
                if (names[i][0] == '\0' || !entityBackups.GetArray(names[i], data, sizeof(data)))
                {
                    continue;
                }
                ...
                // 	// PrintToServer("%f %f %f == %f %f %f", pos[0], pos[1], pos[2], data.origin[0], data.origin[1], data[2]);
